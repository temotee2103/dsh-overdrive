...
> - **图形界面（推荐给非技术用户）**：装好插件后，在 **DSH Web 设置 → Plugins → overdrive** 里填 Telegram Bot Token（保存后重启 `dsh web` 生效）。
> - **进程内原生模式**：设置环境变量 `DSH_TELEGRAM_TOKEN=<bot token>`（@BotFather 创建）后重启 `dsh web`，直接在 DSH 里聊 Telegram。
